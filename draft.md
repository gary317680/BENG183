...
<p align="center"> Figure 11. An example of cluster assignment using model-based algorithm. In the figure to the right, note that each datapoint is associated with an uncertainty value. Larger datapoints have higher uncertainty (lower confidence for them to be assigned to their current cluster), and smaller datapoints have lower uncertainty (higher confidence for the current cluster assignments). </p>
